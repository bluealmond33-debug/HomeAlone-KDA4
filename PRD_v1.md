...
자취생은 냉장고에 남은 재료가 있어도 만들 수 있는 음식을 바로 떠올리기 어렵다. 일반 레시피 검색에서는 조리시간이 길거나 난이도가 높은 결과가 함께 노출되어 실제로 따라 하기 쉬운 메뉴를 찾는 데 시간이 든다. 본 서비스는 보유 재료를 출발점으로 검색하고, 1~2인분·30분 이내·초급 또는 아무나 난이도의 레시피를 우선 제공해 메뉴 선택 시간을 줄인다.
...
| 검색 피로 | 긴 시간·높은 난이도 레시피가 혼재 | 1~2인분, 30분 이내, 초급/아무나 필터 |
...
- 1~2인분, 30분 이내, 초급 또는 아무나 난이도의 레시피를 우선 추천한다.
...
| 조건 준수율 | 1~2인분·30분 이내·초급/아무나를 모두 만족한 추천 비율 | 100% |
...
| 항목 | 필수 | 설명 | 예시 |
|---|---:|---|---|
| 보유 식재료 | Y | 쉼표 또는 줄바꿈으로 구분 | `김치, 밥, 계란` |
| 음식 카테고리 | Y | 한식·중식·일식·양식·분식·상관없음 | `한식` |
| 재추천 요청 | N | 버튼 또는 자연어 의도 | `다시 추천해줘` |
| 변경 조건 | N | 재추천 전에 수정한 재료·카테고리 | `일식`, `김치 제외` |
...
|---|---|---|---|---|---|
| 식재료 검증 | 원문 입력 | 분리·정규화·실제 재료 판별 | 유효 재료, 제외 항목과 이유 | `ingredient_validator_tool` | Must |
...
```json
{"raw_ingredients": "김치, 밥, 계란, 핸드폰"}
```

**처리**

1. 쉼표·전각 쉼표·줄바꿈으로 분리한다.
2. 공백 제거, 연속 공백 축소, 별칭 치환(`달걀 → 계란`)을 수행한다.
3. 입력 순서를 유지하며 중복을 제거한다.
4. 로컬 식재료 사전과 일치하는 항목은 즉시 유효 처리한다.
5. 사전에 없는 항목만 OpenAI에 구조화 판별을 요청한다.
6. 낮은 신뢰도·모호한 항목은 유효 재료로 추측하지 않고 제외 또는 확인 필요로 분류한다.
...
**검증 규칙:** 전체 300자, 최대 20개 항목. 유효 재료가 0개면 이후 Tool을 호출하지 않는다. OpenAI 장애 시 사전 일치 항목만 사용하고 나머지는 `판별 보류`로 안내한다.
...
7. 필수 필드가 없거나 조건을 파싱할 수 없으면 후보를 제외하고 원인을 남긴다.
...
flowchart TD
    ...
    D --> F["1~2인분·30분·초급 필터 및 재료 일치 정렬"]
    F -->|"적합 후보 없음"| E2
...
5. 서비스 계층이 1~2인분·30분·초급/아무나 조건을 필터링한다.
...
| 식재료 입력 | `gr.Textbox` | 쉼표/줄바꿈 입력, 최대 300자 |
...
| 조건 불일치 | 3인분 이상·30분 초과·중급 이상 | 별도 노출 없음 | 후보 제외 |
...
3. scraper가 각 후보의 인분·시간·난이도를 파싱한다.
4. 3인분, 30분 초과, 중급 후보가 제거된다.
...
| 구분 | 입력·조건 | 기대 결과 |
|---|---|---|
| 정상 재료 | `김치, 밥, 계란` | 3개 유효 재료 |
| 중복·별칭 | `계란, 달걀, 계란` | `계란` 1개 |
| 비식재료 | `김치, 핸드폰` | 김치 유효, 핸드폰 제외 |
| 빈 값 | 빈 문자열·공백·None | 사용자 입력 오류 |
| 경계값 | 20개/21개 재료 | 20개 허용, 21개 거부 |
...
- 추천은 1~2인분, 30분 이내, 초급/아무나 조건을 만족한다.
